...
public class BoardDAO {
	...
	public int selectBoardTotalCnt(RangeDTO rDTO) throws SQLException {
		...
		try {
			...
			if (rDTO.getKeyword() != null && !rDTO.getKeyword().isEmpty()) {
				selectTotal.append(" where instr(").append(rDTO.getFieldStr()).append(",?) != 0");
			} // end if
	...
	public List<BoardDTO> selectRangeBoard(RangeDTO rDTO) throws SQLException {
		...
		try {
			...
			selectBoard.append("SELECT num, title, input_date, cnt, id, ip ")
					.append("FROM (SELECT NUM, TITLE, INPUT_DATE, CNT, ID, ip, ")
	...
	public BoardDTO selectBoardDetail(int num) throws SQLException {
		...
		try {
			...
			con = dbCon.getConn();
			// 4.쿼리문 생성 객체 얻기
			String selectDetail = "SELECT TITLE, CONTENT, INPUT_DATE, IP, CNT, ID FROM BOARD WHERE NUM = ?";
	...
	public void updateBoardCnt(int num) throws SQLException {
		...
		try {
			...
			con = dbCon.getConn();
			// 4.쿼리문 생성 객체 얻기
			String updateCnt = " update board set cnt = cnt + 1 where num = ? ";
