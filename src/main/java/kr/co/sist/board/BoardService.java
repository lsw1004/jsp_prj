...
public class BoardService {
	...
	/**
	 * 시작 번호, 끝 번호, 검색필드, 검색 키워드를 사용한 게시글 검색
	 * 
	 * @param rDTO
	 * @return
	 */
...
//		System.out.println("=========");
	...
	public BoardDTO searchOneBoard(int num) {
		...
		try {
			...
			// modifyBoardCnt(num);// boardDetailFrm.jsp나 여기서 처리할것
		} catch (SQLException e) {
			e.printStackTrace();
		} // end catch
		return bDTO;
	...
	public String pagination2(RangeDTO rDTO, String justify) {
		...
		if (rDTO.getTotalPage() > endPage) { // 뒤에 페이지가 더 있음.
			movePage = endPage + 1;
			nextMark.delete(0, nextMark.length());
			nextMark.append("<li class='page-item page-link'><a href='").append(rDTO.getUrl()).append("?currentPage=")
					.append(movePage);
			if (rDTO.getKeyword() != null && !rDTO.getKeyword().isEmpty()) {
		...
		pagination.append("<nav aria-label='...'>").append("  <ul class='pagination d-flex justify-content-")
				.append(justify).append("'>");
		pagination.append(prevMark).append(pageLink).append(nextMark);
		pagination.append("</ul>").append("  </nav>");
	...
	public boolean modifyBoard(BoardDTO bDTO) {
		...
		try {
			flag = bDAO.updateBoard(bDTO)==1?true:false;
		} catch (SQLException e) {
			e.printStackTrace();
		} // end catch
		return flag;
	...
	public boolean removeBoard(BoardDTO bDTO) {
		...
		try {
			flag = bDAO.deleteBoard(bDTO)==1?true:false;
		} catch (SQLException e) {
			e.printStackTrace();
		} // end catch
		return flag;
