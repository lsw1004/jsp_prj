<%@page import="kr.co.sist.emp.EmpDTO"%>
		...
		$.ajax({
			...
			success : function(jsonObj) {
				...
				$.each(jsonArr, function(ind, jsonTemp) {
					createTr = "<tr><td>" + (ind + 1)
							+ "</td><td><a href= 'javascript:empDetail("
							+ jsonTemp.empno + ")'>" + jsonTemp.ename
							+ "</a></td><td>" + jsonTemp.job + "</td><td>"
							+ jsonTemp.sal + "</td><td>"
		...
		<div class="container marketing">
			...
			<div class="row featurette">
				<div class="col-md-7">
					...
					<div>
						<label>부서</label> <select id="dept" style="height: 30px;">
							<option value="N/A">---부서선택---</option>
							<c:forEach var="deptDTO" items="${ deptList }" varStatus="i">
								<option value="${ deptDTO.deptno }"><c:out
										value="${deptDTO.dname }" /></option>
							</c:forEach>
						</select> <input type="button" value="검색" class="btn btn-success btn-sm"
							id="btn">
					</div>
				</div>
				<div id="output">
					<div>
						조회일자 : <span id="pubDate">0000-00-00</span> / 사원수 [<span
							id="empCnt"></span>]
					</div>
