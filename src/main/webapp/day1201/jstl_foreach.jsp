      ...
      <div class="container marketing">
         ...
         <div class="row featurette">
            ...
            %>
           	...
           	<c:if test="${ i.last }">
           	<tr>
           	<td colspan="2">데이터를 출력한 후에 제공할 내용.</td>
           	</tr>
           	</c:if>
           	</c:forEach>
           	</tbody>
           	...
           	<div>
           	<strong>java.util.List</strong><br>
           	다음 중 고객정보가 가장 많이 털린 사이트는?<br>
           	<c:forEach var="site" items="${ list }" varStatus="i">
           	<c:out value="${ i.count }"/>.
           	<input type="radio" value="${ site }" name="site">
           	<c:out value="${ site }"/><br>
           	</c:forEach>
           	</div>
            ...
            pageContext.setAttribute("memebersData", dtoList);
            %>
            <div>
            ...
            <tbody>
            <c:forEach var="member" items="${ memebersData }" varStatus="i">
            <c:set var="ageSum" value="${ member.myAge + ageSum}"/>
            <%-- <c:out value="${ member }"/><br> : 주소가 출력 --%>
            <tr>
            <td><c:out value="${ i.count }"/></td>
            <td><input type="text" name="name" value="${ member.name }"> </td>
            <td><input type="text" name="name" value="${ member.myAge }"> </td>
            </tr>
            <c:if test="${ i.last }">
            <tr>
            <td colspan="2">나이 합계</td>
            <td><c:out value="${ ageSum }"/>살</td>
            </tr>
            </c:if>
            </c:forEach>
            </tbody>
