      ...
      <div class="container marketing">
         ...
         <div class="row featurette">
            <div class="col-md-7">
               ...
               pDTO.setId((String)session.getAttribute("userId"));
               pDTO.setEmail(mail+"@"+domain);
               pDTO.setBirth(birth);
               pDTO.setLoc(loc);
               pDTO.setIntro(intro);
               pDTO.setProfile(profile);
               
               // 프로필 이미지가 선택되지 않으면 기존 이미지명으로 설정되고,
               // 그렇지 않으면 선택한 파일명으로 선택된다.
               ...
               pageContext.setAttribute("flagModify", flagModify);
               
               // index.jsp에서 프로필 이미지를 세션에서 가져와서 보여주기 때문에
               // 회원 정보 변경이 성공하면 세션에 이미지를 변경해준다.
               if(flagModify){
            	   session.setAttribute("userProfile", pDTO.getProfile());
            	   session.setAttribute("birth", pDTO.getBirth());
               }// end if
               %>
               <c:choose>
               <c:when test="${flagModify}">
               회원정보가 수정되었습니다.<br>
               <img src="${CommonURL }/upload/img_2.jpg"/>
               <a href="${CommonURL }/index.jsp">메인화면</a>
               </c:when>
               
               <c:otherwise>
               회원 정보 변경 중 문제가 발생하였습니다.<br>
               사용 중 불편을 드려 대단히 죄송합니다.<br>
               </c:otherwise>
               </c:choose>
            </div>
